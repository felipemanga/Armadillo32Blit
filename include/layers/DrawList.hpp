...
namespace Graphics {
    namespace _drawListInternal{
        ...
        inline void _print(c8 index) {
            ...
            if(u32(textY) >= screenHeight || s32(textY + h*scale) < 0 || u32(textX) >= screenWidth || s32(textX + w*scale) < 0 || isWhitespace) {
...
*/
    ...
    inline bool draw(const BitmapLike &data, Point2D topLeft, f32 falpha = 1){
        topLeft -= camera;
        return draw<transparent, scale2x, BitmapLike::bitsPerPixel, false>(data, round(topLeft.x), round(topLeft.y), falpha);
    }
